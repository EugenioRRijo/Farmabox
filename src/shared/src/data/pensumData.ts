/**
 * Pensum Data for Facultad de Farmacia USM
 * Based on PENSUM 2023-01 (1).pdf
 */

export interface PensumSubject {
  code: string;
  name: string;
  credits: number;
  hasLab: boolean;
  hoursTheory: number;
  hoursLab: number;
  prerequisites: string[];
  professors?: string[];
  labNumber?: string; // Número de laboratorio (ej: "104", "300")
}

export interface Semester {
  number: number;
  subjects: PensumSubject[];
}

/**
 * Complete Pensum Data - 10 Semesters
 */
export const PENSUM_DATA: Semester[] = [
  // SEMESTRE 1
  {
    number: 1,
    subjects: [
      {
        code: '3307011101',
        name: 'Orientación Farmacéutica I',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: [],
      },
      {
        code: '3307011103',
        name: 'Física Aplicada I',
        credits: 3,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: [],
        labNumber: '104',
      },
      {
        code: '3307011105',
        name: 'Matemática Aplicada I',
        credits: 4,
        hasLab: false,
        hoursTheory: 4,
        hoursLab: 0,
        prerequisites: [],
      },
      {
        code: '3307011108',
        name: 'Biología I',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 2,
        prerequisites: [],
        labNumber: '101',
      },
      {
        code: '3307012101',
        name: 'Química Básica I',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: [],
        labNumber: '300',
      },
      {
        code: '3307014213',
        name: 'Botánica Aplicada I',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: [],
      },
    ],
  },
  // SEMESTRE 2
  {
    number: 2,
    subjects: [
      {
        code: '3307011201',
        name: 'Orientación Farmacéutica II',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: ['3307011101'],
      },
      {
        code: '3307011204',
        name: 'Matemática Aplicada II',
        credits: 4,
        hasLab: false,
        hoursTheory: 4,
        hoursLab: 0,
        prerequisites: ['3307011105'],
      },
      {
        code: '3307011206',
        name: 'Biología II',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 2,
        prerequisites: ['3307011108'],
        labNumber: '101',
      },
      {
        code: '3307012201',
        name: 'Química Básica II',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307012101'],
        labNumber: '300',
      },
      {
        code: '3307014214',
        name: 'Botánica Aplicada II',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 3,
        prerequisites: ['3307014213'],
      },
      {
        code: '3307011203',
        name: 'Física Aplicada II',
        credits: 3,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307011103'],
        labNumber: '104',
      },
    ],
  },
  // SEMESTRE 3
  {
    number: 3,
    subjects: [
      {
        code: '3307012301',
        name: 'Química Orgánica I',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307012201'],
        labNumber: '401',
      },
      {
        code: '3307011302',
        name: 'Anatomía',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 2,
        prerequisites: ['3307011206'],
        labNumber: '101',
      },
      {
        code: '3307012302',
        name: 'Química Analítica I',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307012201'],
        labNumber: '300',
      },
      {
        code: '3307011304',
        name: 'Estadística',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: ['3307011204'],
      },
      {
        code: '3307011305',
        name: 'Fisicoquímica I',
        credits: 4,
        hasLab: true,
        labNumber: '104',
        hoursTheory: 3,
        hoursLab: 2,
        prerequisites: ['3307011203', '3307012201'],
      },
      {
        code: '3307031107',
        name: 'Estudio y Comprensión del Hombre',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: [],
      },
      {
        code: '3307032103',
        name: 'Química Inorgánica I',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307022102'],
        labNumber: '302',
      },
      {
        code: '3307038107',
        name: 'Atención Farmacéutica I',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: ['3307021102'],
      },
    ],
  },
  // SEMESTRE 4
  {
    number: 4,
    subjects: [
      {
        code: '3307012401',
        name: 'Química Orgánica II',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307012301'],
        labNumber: '401',
      },
      {
        code: '3307011401',
        name: 'Fisiología',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 2,
        prerequisites: ['3307011302'],
        labNumber: '101',
      },
      {
        code: '3307012402',
        name: 'Química Analítica II',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307012302'],
        labNumber: '300',
      },
      {
        code: '3307011403',
        name: 'Bioquímica I',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307012301'],
        labNumber: '101',
      },
      {
        code: '3307011402',
        name: 'Fisicoquímica II',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 2,
        prerequisites: ['3307011305'],
        labNumber: '104',
      },
      {
        code: '3307047105',
        name: 'Metodología de la Investigación',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: ['3307011304'],
      },
      {
        code: '3307042104',
        name: 'Química Inorgánica II',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307032103'],
        labNumber: '302',
      },
      {
        code: '3307048108',
        name: 'Atención Farmacéutica II',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: ['3307038107'],
      },
    ],
  },
  // SEMESTRE 5
  {
    number: 5,
    subjects: [
      {
        code: '3307013501',
        name: 'Farmacognosia I',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307012401', '3307014214'],
        labNumber: '304',
      },
      {
        code: '3307011501',
        name: 'Bioquímica II',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307011403'],
        labNumber: '101',
      },
      {
        code: '3307011502',
        name: 'Microbiología',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307011206', '3307012201'],
        labNumber: '204',
      },
      {
        code: '3307011503',
        name: 'Parasitología',
        credits: 3,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307011206'],
      },
      {
        code: '3307013502',
        name: 'Farmacia Galénica I',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 4,
        prerequisites: ['3307011402'],
        labNumber: '105',
      },
      {
        code: '3307052105',
        name: 'Química Medicinal I',
        credits: 4,
        hasLab: true,
        labNumber: '401',
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307012401'],
      },
      {
        code: '3307053103',
        name: 'Fisiopatología I',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: ['3307011401'],
      },
      {
        code: '3307053205',
        name: 'Salud Pública I',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: [],
      },
      {
        code: '3307057103',
        name: 'Diseño de Proyectos I',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: ['3307047105'],
      },
    ],
  },
  // SEMESTRE 6
  {
    number: 6,
    subjects: [
      {
        code: '3307013601',
        name: 'Farmacognosia II',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307013501'],
        labNumber: '304',
      },
      {
        code: '3307013602',
        name: 'Farmacia Galénica II',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 4,
        prerequisites: ['3307013502'],
        labNumber: '107',
      },
      {
        code: '3307011601',
        name: 'Farmacología I',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 2,
        prerequisites: ['3307011501', '3307011401'],
        labNumber: '302',
      },
      {
        code: '3307011602',
        name: 'Inmunología',
        credits: 3,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307011502'],
      },
      {
        code: '3307012601',
        name: 'Análisis de Medicamentos I',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 4,
        prerequisites: ['3307012402'],
        labNumber: '200',
      },
      {
        code: '3307062106',
        name: 'Química Medicinal II',
        credits: 4,
        hasLab: true,
        labNumber: '401',
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307052105'],
      },
      {
        code: '3307063104',
        name: 'Fisiopatología II',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: ['3307053103'],
      },
      {
        code: '3307063206',
        name: 'Salud Pública II',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: ['3307053205'],
      },
      {
        code: '3307067104',
        name: 'Diseño de Proyectos II',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: ['3307057103'],
      },
    ],
  },
  // SEMESTRE 7
  {
    number: 7,
    subjects: [
      {
        code: '3307013701',
        name: 'Tecnología Farmacéutica I',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 4,
        prerequisites: ['3307013602'],
      },
      {
        code: '3307011701',
        name: 'Farmacología II',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 2,
        prerequisites: ['3307011601'],
        labNumber: '302',
      },
      {
        code: '3307012701',
        name: 'Análisis de Medicamentos II',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 4,
        prerequisites: ['3307012601'],
        labNumber: '406',
      },
      {
        code: '3307011702',
        name: 'Toxicología I',
        credits: 3,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307011601'],
        labNumber: '406',
      },
      {
        code: '3307011703',
        name: 'Bromatología',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 3,
        prerequisites: ['3307011502', '3307012402'],
        labNumber: '409',
      },
      {
        code: '3307076101',
        name: 'Economía Aplicada',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: [],
      },
      {
        code: '3307075109',
        name: 'Higiene y Seguridad Industrial I',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: [],
      },
      {
        code: '3307078101',
        name: 'Orientación Pasantías Oficina de Farmacia',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: ['3307065106'],
      },
    ],
  },
  // SEMESTRE 8
  {
    number: 8,
    subjects: [
      {
        code: '3307013801',
        name: 'Tecnología Farmacéutica II',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 4,
        prerequisites: ['3307013701'],
      },
      {
        code: '3307011801',
        name: 'Farmacología III',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 2,
        prerequisites: ['3307011701'],
      },
      {
        code: '3307012801',
        name: 'Análisis de Medicamentos III',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 4,
        prerequisites: ['3307012701'],
      },
      {
        code: '3307011802',
        name: 'Toxicología II',
        credits: 3,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307011702'],
        labNumber: '406',
      },
      {
        code: '3307013802',
        name: 'Biofarmacia',
        credits: 3,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307013701', '3307011701'],
        labNumber: '200',
      },
      {
        code: '3307086102',
        name: 'Administración Aplicada',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: ['3307076101'],
      },
      {
        code: '3307088102',
        name: 'Pasantías I Oficina de Farmacia',
        credits: 4,
        hasLab: false,
        hoursTheory: 0,
        hoursLab: 6,
        prerequisites: ['3307078101'],
      },
      {
        code: '3307085110',
        name: 'Seguridad e Higiene Industrial II',
        credits: 3,
        hasLab: false,
        hoursTheory: 3,
        hoursLab: 0,
        prerequisites: ['3307075109'],
      },
    ],
  },
  // SEMESTRE 9
  {
    number: 9,
    subjects: [
      {
        code: '3307013901',
        name: 'Tecnología Farmacéutica III',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 4,
        prerequisites: ['3307013801'],
        labNumber: '105',
      },
      {
        code: '3307011901',
        name: 'Farmacia Clínica',
        credits: 4,
        hasLab: true,
        hoursTheory: 3,
        hoursLab: 2,
        prerequisites: ['3307011801'],
      },
      {
        code: '3307011902',
        name: 'Control de Calidad',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 4,
        prerequisites: ['3307012801'],
      },
      {
        code: '3307011903',
        name: 'Farmacia Hospitalaria',
        credits: 3,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307011801'],
      },
      {
        code: '3307011904',
        name: 'Legislación Farmacéutica',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: [],
      },
      {
        code: '3307096103',
        name: 'Mercadotecnia I',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: [],
      },
      {
        code: '3307093209',
        name: 'Primeros Auxilios I',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: ['3307084106'],
      },
      {
        code: '3307096105',
        name: 'Técnicas Gerenciales I',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: ['3307086102'],
      },
      {
        code: '3307095111',
        name: 'Dermocosmética I',
        credits: 3,
        hasLab: true,
        labNumber: '203',
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307084108'],
      },
      {
        code: '3307097106',
        name: 'Seminario Trabajo Especial de Grado I',
        credits: 3,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: ['3307067104'],
      },
      {
        code: '3307098103',
        name: 'Orientación Pasantías Industriales-Empresas',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: ['3307088102'],
      },
      {
        code: '3307095107',
        name: 'Farmacotecnia III',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 4,
        prerequisites: ['3307085104'],
        labNumber: '105',
      },
      {
        code: '3307094211',
        name: 'Farmacoterapéutica I',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307084104'],
      },
    ],
  },
  // SEMESTRE 10
  {
    number: 10,
    subjects: [
      {
        code: '3307011001',
        name: 'Pasantías',
        credits: 12,
        hasLab: false,
        hoursTheory: 0,
        hoursLab: 40,
        prerequisites: [],
      },
      {
        code: '3307011002',
        name: 'Trabajo Especial de Grado',
        credits: 6,
        hasLab: false,
        hoursTheory: 0,
        hoursLab: 0,
        prerequisites: [],
      },
      {
        code: '3307106104',
        name: 'Mercadotecnia II',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: ['3307096103'],
      },
      {
        code: '3307103210',
        name: 'Primeros Auxilios II',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: ['3307093209'],
      },
      {
        code: '3307106106',
        name: 'Técnicas Gerenciales II',
        credits: 2,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: ['3307096105'],
      },
      {
        code: '3307105112',
        name: 'Dermocosmética II',
        credits: 3,
        hasLab: true,
        labNumber: '203',
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307095111'],
      },
      {
        code: '3307107107',
        name: 'Seminario Trabajo Especial de Grado II',
        credits: 3,
        hasLab: false,
        hoursTheory: 2,
        hoursLab: 0,
        prerequisites: ['3307097106'],
      },
      {
        code: '3307105108',
        name: 'Farmacotecnia IV',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 4,
        prerequisites: ['3307095107'],
        labNumber: '105',
      },
      {
        code: '3307104212',
        name: 'Farmacoterapéutica II',
        credits: 4,
        hasLab: true,
        hoursTheory: 2,
        hoursLab: 2,
        prerequisites: ['3307094211'],
      },
    ],
  },
];

/**
 * Helper function to get subjects by semester
 */
export function getSubjectsBySemester(semester: number): PensumSubject[] {
  const found = PENSUM_DATA.find((s) => s.number === semester);
  return found ? found.subjects : [];
}

/**
 * Helper function to get subject by code
 */
export function getSubjectByCode(code: string): PensumSubject | undefined {
  for (const semester of PENSUM_DATA) {
    const subject = semester.subjects.find((s) => s.code === code);
    if (subject) return subject;
  }
  return undefined;
}

/**
 * Helper function to get all subjects
 */
export function getAllSubjects(): PensumSubject[] {
  return PENSUM_DATA.flatMap((s) => s.subjects);
}
